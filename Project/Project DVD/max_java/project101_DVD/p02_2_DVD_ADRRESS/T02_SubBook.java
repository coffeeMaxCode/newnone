...
public class T02_SubBook extends JDialog implements ActionListener {
	...
	String title = null;
	T01_AddressBook aBook = null;
	JScrollPane jsp_line = new JScrollPane(jp_center);
	T03_AddressVO aVO = new T03_AddressVO();

	public T02_SubBook() {
		initDisplay();
	};

//	public T02_SubBook(T01_AddressBook aBook) {  
//		this.aBook =aBook;     
//	}
	/***********************************************************************************
	 * 부모창에서 결정된 값(입력 aVO)
	 * @param aVO 입력버튼 부모창에서 눌렀을 때, null // 수정버튼을 눌렀을 때 aVO는 자료의 주소번지를 갖게됨
	 * @param title 부모창에서 선택한 버튼 라벨 담기
	 * @param aBook 부모창AddressBook의 원본 주소번지 담는 변수
	 * @param isEdit 자식창SubBook을 활성화 하는 것을 담는 변수
	 **********************************************************************************/
	public void set(T03_AddressVO aVO, String title, T01_AddressBook aBook, boolean isEdit) {
		this.aVO = aVO;
		this.title = title;
		this.aBook = aBook;
		this.setValue();
 ...
 *******************************************************************/
	public void setValue() {
		...
		//상세조회나 수정일 때 aVO에 있는 값으로 각 콤포넌트(txtId, txtName..)를 초기화한다.
		else {
			//JOptionPane.showMessageDialog(aBook, "aVO:"+ aVO.getId());
