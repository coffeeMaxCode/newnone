...
public class ExcelFrame extends JInternalFrame {
  ...
  * 예를들어 setValueAt(값,로우 index, 컬럼 index)
  *    getValueAt(로우 index,컬럼 index)
  * 또한 DefaultTableModel은 특정 셀이 편집될 수 있는지 각 열의 데이터 타입은 무엇인지를 얻어내는 역할도 함
  * JTable의 DefaultTableModel에 있는 데이터 위치는 JTable의 화면에 디스플레이 되었을 때 위치와 일치하지 않는다.
  */
 DefaultTableModel dtm = new DefaultTableModel(100,27);//100행 27열
 
 /*
  * DefaultTableColumnModel 은 TableColumn의 인스턴스를 관리하기 위해 디자인됨.
  *  TableColumn 은 DefaultTableColumnModel 데이터의 한 열을 표현하여
  *  실제 JTable GUI에 보여지는 열을 관리하는데 필요함.
  */
 DefaultTableColumnModel dtcm = new DefaultTableColumnModel();
 
 /*
  * DefaultListSelectionModel은 열의 순서 , 선택 , 여백 등 TableColumn들을 관리함.
  * 열을 선택하는 방법을 여러가지로 지원하기 위해 ListSelectionModel을 사용
  */
 ...
 /*
  * DefaultTableCellRenderer 는 TableCellRenderer 인터페이스를 구현한 디폴트 클래스이다.
 ...
 /*
  * DefaultCellEditor는 tableCellEditor 인터페이스와 TreeCellEditor인터페이스 모두를 구현한다.
 ...
 public void initDisplay() {
  tc[0] = new TableColumn(0,75,dtcr,dce);//테이블의 첫번째(인덱스 0)헤더정보 설정하기 위한 코드
